      ...
      {books.length>0?books.map((item) => {
        return (
          ...
          <div key={item._id}
            ...
            <button onClick={()=>{handleCart(item._id)}} className='w-full justify-center bg-yellow-500'>Add to cart<i class="bi bi-cart"></i> </button>
            </div>
          
          </div>
        )
      }):
      <div className='m-auto text-xl md:text-2xl'>Loading...</div>
      }
